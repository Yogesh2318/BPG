...
export  const  getRequest = async (req,res)=>{
    ...
    try {
        console.log("Inside getRequest controller", req.user);
...
export  const  comepeteFriend = async (req,res)=>{

    try {
        console.log("Inside comepeteFriend controller", req.user);
        const id = req.user._id;
        ...
        const competeFriend = await User.findById(competeId);
        if (!competeFriend) {
            return res.status(404).json({ message: "Compete Friend not found" });
        }
        const userNumberOfPlants = user.plants.length;
        const competeFriendNumberOfPlants = competeFriend.plants.length;
        res.status(200).json({ 
            yourPlants: userNumberOfPlants, 
            friendPlants: competeFriendNumberOfPlants 
        });
    }   
    catch (error) {
        res.status(500).json({ message: error.message });
